(async function () {
  ...
  if (!currentUrl.includes("Section=CEPZ-Import") || !currentUrl.includes("Import")) {
  ...
  const CONTROL_URL = "https://script.google.com/macros/s/AKfycbzB44ZIJ_iZK1DAlbSi48MscUi7dzRmayTj-s7911C_qBIfaKIA6uGwHSnCnnGagcWLBQ/exec";
  const API_URL = "https://script.google.com/macros/s/AKfycbyAQnnqm2Ci2evLAcIbcwDnORxZGSVAWmXn8MpjXYq3CylipEeXmBL0wVMbuqk_-Aka1w/exec";
  ...
  async function runMainScript() {
    try {
      ...
      let cachedData = JSON.parse(localStorage.getItem('receipt_perfect_cacheimport'));
      ...
      if (incomingData && incomingData.length > 0) {
        if (!cachedData || JSON.stringify(cachedData) !== JSON.stringify(incomingData)) {
          localStorage.setItem('receipt_perfect_cacheimport', JSON.stringify(incomingData));
